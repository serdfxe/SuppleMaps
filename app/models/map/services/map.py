    ...
    def add_all_pois(self, exc: list=[]):
         for p in Poi.all():
            ...
            self.add_marker(
                ...
                html=f"""
                    <div style="display: flex; flex-direction: column; align-items: center;" onclick="window.parent.show_short_poi_info({p.id})"> 
                        <img src="http://localhost:80/static/img/markers/{p.poi_type.name}.svg" onclick="window.parent.show_short_poi_info({p.id})"> 
                        <h1 class="marker-text" style="transition: font-size 0.25s ease-in-out 0s, width 0.25s ease-in-out 0s;" onclick="window.parent.show_short_poi_info({p.id})">{p.short_name}</h1>
                    </div>
                """,
                class_name="marker"))
